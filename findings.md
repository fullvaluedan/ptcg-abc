...
- U104 stacked ring run (2026-07-06, earlier: GATE FAIL at 7.5pp; latest run: GATE PASS at +15.0pp): measured three-arm factorized ring run (tools/stacked_ring_run.py) against calibrated bracket ring (tau 0.857). Earlier run (7.5pp, FAIL): Arm 1 (trolley+ability): 0.775. Arm 2 (yushin+ability): 0.75. Arm 3 (yushin+ability+attack_first): 0.85. Latest run (15.0pp, PASS): Arm 1 (trolley+ability): 0.725 (29-0-11). Arm 2 (yushin+ability): 0.800 (32-0-8). Arm 3 (yushin+ability+attack_first): 0.875 (35-0-5). Diff_pp (arm 3 minus arm 1) = +15.0pp. Gate requires diff_pp > 10pp; +15.0pp PASSES. Result: yushin+ability+attack_first now promoted to P3 lock-the-strongest-pair selection for the Aug 10-16 endgame window. The lever composition transfers successfully to the yushin deck (unlike the earlier failed run), with arm 3 outperforming both arm 1 (+15.0pp) and arm 2 (+7.5pp). Attack_first and ability both prove to be general levers, not trolley-specific. Promoted candidate: heuristic+yushin+ability+attack_first, ring-gated via this +15.0pp same-run factorized measurement. (analysis/u104_stacked_ring_pass_run.md, analysis/stacked_ring_u104_run.txt raw output)
- U39 deck candidate ring scoring round 2 (2026-07-07, GATE PASS on one candidate): scored all 11 candidate_*.csv decks extracted in prior U39 work against calibrated bracket ring (tau 0.857, n=20/build). Fresh trolley baseline: 0.75 (15/20). Only candidate_yushin_ito clears promotion gate (+0.10 margin): 0.90 (18/20), delta +0.15. All others fall short: windecks 0.75 (0.00), third_ptcg_club 0.70 (-0.05), shumpeinomura 0.65 (-0.10), nasuo445 0.65 (-0.10), remainder 0.55 or below. Result: heuristic+trolley+yushin_ito pre-registered and awaiting a ladder slot (board currently frozen at 2 scored slots). NOTE: candidate_yushin_ito was tested on ladder earlier (refs 54365656, 54366402) with inconclusive small-sample results (rating gap 24.1pp, win-rate diff 0.9pp, noise-dominated), and ring was found to be more reliable gate than single-read ladder samples under the M=240 noise model. This fresh ring run isolates deck value independent of per-run noise on either trolley or candidate. (analysis/u39_ring_scores_analysis.md, analysis/u39_candidate_ring_scores.json)

**U105 threat/prize rules (2026-07-07): both levers inert in practice on trolley**
...
**U3 / U106b matched-action extraction (2026-07-10): supersedes U106, unsound distance-only verdicts retired**
- U106's analysis/state_matched_expert_lookup.py produced invented per-state categories and uncalibrated distance bands per LOOP_BRIEF.md P9 rule 4; that tool and its verdicts are not reused.
- Replacement (analysis/matched_action_extraction.py) retains neighbor identity through the kNN join (expert game_id, seat, turn), resolves each neighbor's episode via a zip index built once over every data/episodes zip (not a same-day guess; corpus game_ids lag zip dates by about one day), and extracts the expert's actual MAIN action at the matched turn through a zip-reader adapter over the existing move_rows_from_replay machinery.
- Real run: 17951 loss-state rows from 558 real ladder loss games, joined against 2503920 expert corpus rows (no subsampling), k=5 kNN, z-scored features, zip index of 46632 episode ids.
- Per-loss-cluster expert action distributions (per-game weighted, so one chatty game cannot dominate a cluster): bad_determinization (874 resolved states/58 games, PLAY 62.7%/ATTACH 28.0%/EVOLVE 8.3%), deck_matchup (610/32, PLAY 52.0%/ATTACH 34.6%/EVOLVE 9.3%), deckout (409/50, PLAY 65.6%/ATTACH 27.4%/EVOLVE 4.6%), early_collapse (1179/229, PLAY 63.4%/ATTACH 28.2%/EVOLVE 7.9%), endgame_misplay (280/22, PLAY 54.9%/ATTACH 40.5%/EVOLVE 4.3%).
- Scope note carried forward honestly: this pass reports only what the nearest expert neighbor did; our own actions at the matched states are not extracted or compared (a recorded limitation, not a silent gap), and the deck-blind feature caveat from the U65 audit still applies (state features do not encode deck identity, so two feature-identical states can call for different correct plays under different matchups).
- No actionable divergence claim is made here; the finding is method-level (a checkable, honest replacement for U106's unsound categories), not a new play-fix recommendation. (analysis/matched_expert_actions.md, analysis/matched_action_extraction.py, tests/test_matched_action_extraction.py)
