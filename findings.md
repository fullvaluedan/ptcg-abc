...
**Queue item 4: Deck candidate exploration (U39), 2026-07-05**. Mined dataset produced 6 candidate decks. Scored through calibrated bracket ring (tau 0.857, n=20 each, 9 opponents): candidate_yushin_ito 90.0% vs trolley baseline 80.0% (+10.0pp, material margin per gate). All other 5 candidates underperformed trolley (45-75%). Candidate_yushin_ito passes pre-registration gate; others fail. Known blocker: candidate_yushin_ito has documented ERROR on ladder (ref 54362805, escalated, root cause unknown). Recommendation: resolve escalation error before re-submission, or wait for error diagnosis. (analysis/candidate_deck_ring_scoring.md)
