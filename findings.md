...
- U101 invariant fuzzer discovery (2026-07-06): ran tools/invariant_fuzzer.py on 50 games loaded from replay data. All five invariant checks passed (HP bounds, prize bounds, turn counter, energy flags) in 0/50 violations. Card conservation check found systematic violations in 49/50 games. Pattern: Player 0 consistently holds 66 cards (6 excess), Player 1 consistently holds 59 cards (1 deficit) at game-init state loaded from replays. Card zones counted: deck, hand, discard, active, bench, prize, tools, energyCards. Root cause remains ambiguous: either the count is missing a zone (select.deck, preEvolution, stadium), or the games loaded from replay files are mid-game states with inherent card accounting discontinuities at replay boundaries, or the 60-card invariant only applies to fresh deck-init states (not mid-replay loads). Interpretation required before treating as bug or rule violation. (analysis/engine_quirks.md, tools/invariant_fuzzer.py, escalated for review pending decision on whether to proceed with violation as TBD or block until root cause is understood)
