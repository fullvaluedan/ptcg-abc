...
class _StateLogger:
    ...
    def flush_game(self, rows, r0, r1):
        for row in rows:
            seat = row[1]
...
def run_gauntlet(agent_name, opponent_names, n_matches, log_states=False, log_path=None):
    ...
    logger = _StateLogger(log_path or DEFAULT_LOG_DIR / f"states_{int(time.time())}.csv") if log_states else None
    try:
        for i in range(n_matches):
            ...
            if logger is not None:
                wrapped_a = logger.wrap(wrapped_a, i, rows)
                wrapped_b = logger.wrap(wrapped_b, i, rows)
            res = run_match(wrapped_a, wrapped_b, swap_first=(i % 2 == 1))
            if logger is not None:
                logger.flush_game(rows, *_absolute_rewards(res))
...
def main():
    ...
    s = run_gauntlet(args.agent, args.opponents, args.matches, log_states=log_states)
    ci = s["win_rate_ci95"]
