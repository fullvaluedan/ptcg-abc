...
| --- | --- | --- | --- | --- | --- |
| gameplan_seeds_diffuse | refuted for target-card seeds; a related within-turn sequencing signal CONFIRMED | 5732 real episodes; grimmsnarl 1996/1763, archaludon 2294/2442 | meta_grimmsnarl + meta_archaludon | MET at the resolver level 2026-07-03 (U91 step 1): attach_target/play_target now resolve the receiving Pokemon / played hand card (were 0.470/0.285 and 0.000 barred); validated on bracket_4 real data (n=1500 episodes) at 1.000/1.000 resolution. 2026-07-03 (U91 step 2): two new turn-scoped blocks, attach_before_attack and energy_banking, both CLEARED the claim gate (n>1400/side, bootstrap 90% CI excludes zero) AND the prediction gate (KD4 train CI brackets the held-out test mean) on bracket_4 -- winners attach-before-attacking 3.4pp less and bank energy 4.4pp less than losers; game_length_turns was CUT (claim CI straddles zero). Still open: (1) these are small, descriptive correlations, U93 must A/B a real rule, not just port them; (2) the archetype-registry shadowing that keeps meta_archaludon/meta_grimmsnarl from classifying any deck in this dataset still blocks a like-for-like re-mine of those two named families. 2026-07-04 (U93 step 2): the bracket-ring A/B gate PASSED both required checks -- offline weak-bot gauntlet +5.5pp (analysis/attack_first_ab.md) and calibrated bracket-ring +10.0pp, agreeing in direction (analysis/attack_first_ring_check.md). Pre-registered as heuristic+trolley-attack_first (state/current.md, direction up, M=60, N=30, settle-by 2026-07-11); staged and grader-verified, awaiting a free ladder slot. | analysis/gameplans/seeds_real_run.md, analysis/gameplan_target_resolution_fixed.md, analysis/gameplan_claims_bracket_4.md, analysis/attack_first_ab.md, analysis/attack_first_ring_check.md |
...
- update 2026-07-03 (U91 step 2): a DIFFERENT question than target-card seeds -- within-turn SEQUENCING -- does carry real, gate-passing signal on bracket_4. `analysis/gameplan_claim_gate.py`'s two honesty gates (claim: n>=200/side, bootstrap 90% CI excludes zero; prediction: KD4 train-mined CI must bracket the held-out test mean) both passed for `attach_before_attack` (win 0.524 vs loss 0.558, a 3.4pp gap) and `energy_banking` (win 0.192 vs loss 0.236, a 4.4pp gap); `game_length_turns` was mined too but CUT (claim CI straddles zero, -0.145 to +0.014). Full dataset, n=1472-1892 per side depending on block. `analysis/gameplan_claims_bracket_4.md`.
- update 2026-07-03 (U93 step 1): built the literal flag-gated rule the plan names from the confirmed gap -- `PTCG_ATTACK_FIRST` (default off) in `agents/heuristics.py`'s `choose()`: when a positive-value attack is already legal THIS decision without a further attach, take it now instead of the discretionary attach. Ran the same fires-vs-inert discipline as `measure_energy_seq`/`measure_bench_dig` before spending a bracket-ring slot: `tools/measure_attack_first.py` captured 20 real trolley ATTACH+ATTACK positions (heuristic pilot, up to 10 matches), 8 with a positive-value attack on the table, 3/20 flipped the end-to-end pilot decision. LIVE, not inert (`analysis/attack_first_flip_check.md`). Still open: the bracket-ring A/B (>=+5pp with gauntlet-direction agreement) before any ladder slot.
- update 2026-07-04 (U93 step 2): ran both required offline gates before spending a ladder slot. Weak-bot gauntlet (`tools/run_ab.py --flag PTCG_ATTACK_FIRST`, deck:trolley vs the standard 8-deck pool, 200 games/arm): off 71.5% (143/57) -> on 77.0% (154/46), diff_pp +5.5, no regression, 0 invalid moves (`analysis/attack_first_ab.md`). Bracket-ring A/B (`tools/attack_first_ring_check.py`, new tool mirroring `ability_ring_check.py`, 20 games/arm against the calibrated U81 ring): off 75.0% -> on 85.0%, diff_pp +10.0, agrees in direction (`analysis/attack_first_ring_check.md`). Both gates PASS (gauntlet must-not-regress + >=+5pp ring margin with directional agreement). Built and grader-verified the tarball (`tests/test_grader_submission.py[heuristic-trolley-attack_first]`, both the in-process and extracted-tarball `env.run` paths), and pre-registered `heuristic+trolley-attack_first` (`state/current.md`, direction up, M=60, N=30, settle-by 2026-07-11). NOT submitted: both ladder slots occupied (trolley king copy + the in-flight L1 ability A/B, settle-by 2026-07-08). Staged to submit the instant a slot frees, per TRACK L priority.
