...
- DEFERRED (not built): the remaining application points the plan U37 names (bench / play targets, fetch priorities, the deckout floor) and the ordered FORCE/VETO guard-stack formalization. The empty seeds channel cannot exercise the extra target points, so building them now would be speculative code; and choose() already implements lethal-force, once-per-turn ability-veto, deckout-veto, thin-bench, and rare-candy ordering. NO ladder slot is spent on a seeds build until a block emits a concentrated AND win-vs-loss-discriminating seed (gameplan_seeds_diffuse re-test conditions).
