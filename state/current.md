...
```json STATE
{
  ...
  "in_flight": {
    ...
    "note": "Board-checked this iteration (kaggle competitions submissions): tracked latest-2 unchanged, ref 54315802 (ability floor restoration) now reads COMPLETE 488.9 (drifted from 564.1) and ref 54315565 (plain king-copy revert) now reads COMPLETE 448.0 (drifted from 502.9). Both readings sit inside the existing v2 pooled range (396.7-691.5), no new low, no drift off the L9-target composition (one ring-preferred build, one plain king copy), so per L9(c)/(d) TRACK L continues to genuinely HOLD; no new submission spent. Fell back to TRACK S: found and fixed a writeup staleness gap in docs/writeup/comprehension.md, whose claims ledger and closing paragraph still described PTCG_ATTACK_FIRST as 'staged, not yet submitted' even though it was submitted (refs 54304483/54304681), settled NEUTRAL via the U23 scoreboard tiebreak, and reverted to a king copy days earlier. Corrected the ledger row and the closing paragraph to state the actual settled-NEUTRAL outcome and note the lever remains re-eligible for a future slot since both offline gates still pass.",
    "ref": "n/a"
  },
  "ledger": [
    {
