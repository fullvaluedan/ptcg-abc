...
```json STATE
{
  ...
  "in_flight": {
    ...
    "note": "Board-checked this iteration (kaggle competitions submissions): tracked latest-2 unchanged, ref 54315802 (ability floor restoration) now reads COMPLETE 564.1 (drifted from 594.7) and ref 54315565 (plain king-copy revert) now reads COMPLETE 502.9 (drifted from 396.7). Both readings sit inside the existing v2 pooled range (396.7-691.5), no new low, no drift off the L9-target composition (one ring-preferred build, one plain king copy), so per L9(c)/(d) TRACK L continues to genuinely HOLD; no new submission spent. Fell back to TRACK S: found and fixed a writeup staleness gap in docs/writeup/offline_ladder_transfer.md, which still cited only the first, ref-scoped noise correction (452-691 on ref 54282104 alone) and never mentioned the subsequent pooled-multi-ref v2 widening (396.7-691.5, margin_M 150) that state/current.md and findings.md already carry. Same class of doc-vs-state drift as the last iteration's noise_model JSON fix, just in the writeup doc instead of the machine-readable block.",
    "ref": "n/a"
  },
  "ledger": [
    {
