...
```json STATE
{
  ...
  "in_flight": {
    ...
    "note": "heuristic+trolley-attack_first SETTLED NEUTRAL 2026-07-03 via the U23 scoreboard tiebreak (analysis/attack_first_settlement.md). Per the pre-registered BAND action the slot reverts to a byte-identical heuristic+trolley king copy. On-disk submission_trolley.tar.gz was STALE (Jul 2 build, missing agents/card_effects.py, would have ERRORed like ref 54281824) so it was rebuilt fresh this iteration (tools/build_submission.py --agent agents/agent_heuristic.py --deck decks/trolley.csv --extra agents/heuristics.py --extra agents/card_effects.py) and re-verified (test_grader_submission.py[heuristic-trolley], 2 passed). Submission attempt via the raw kagglesdk client (patched _prepare_response to surface response.text) confirms the daily quota is STILL exhausted: {\"error\":{\"code\":400,\"message\":\"Submission not allowed:  Your team has used its daily Submission allowance (5) today, please try again tomorrow UTC (6.7 hours from now).\"}}. Real clock (date -u) reads 2026-07-03 17:13 UTC, consistent with a reset at ~00:00 UTC 2026-07-04 (~6.7h out). Tarball is built, grader-verified, and ready; submit it as the first action next iteration once past the reset.",
