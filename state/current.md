...
- cem-grown-genome (PRIO ordering): genome grown so CEM has a gradient (analysis/cem_gradient_restored.md); the tuner's agreement channel now filters to the train md5 bucket (--split train default) so a run cannot contaminate its own held-out test bucket (analysis/cem_split_holdout.md, KD4). The real tools/cem_tune.py run is the pending next step (fit on train, then the pre-registered offline filter on test), then gate on ladder A/B. Un-tuned build ships byte-identical.
...
```json STATE
{
  "active_candidates": [
    ...
    },
    {
      ...
      "note": "genome grown so CEM has a gradient (analysis/cem_gradient_restored.md); the tuner's agreement channel now filters to the train md5 bucket (--split train default) so a run cannot contaminate its own held-out test bucket (analysis/cem_split_holdout.md, KD4). The real tools/cem_tune.py run is the pending next step (fit on train, then the pre-registered offline filter on test), then gate on ladder A/B. Un-tuned build ships byte-identical."
    }
  ],
  "census": {
